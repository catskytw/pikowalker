...
1. **基本權限** — 位置權限、通知權限，第一次打開 PikoWalker 就會跳出來要求，全部允許。
...
4. **Health Connect → Google Fit → 皮克敏 Bloom（如果想同步步數）**
   ...
   **為什麼要繞這三層？**
   PikoWalker 沒辦法直接把步數塞進皮克敏 Bloom 遊戲裡——皮克敏 Bloom 不認得 PikoWalker 這個應用程式，不會讓它直接寫資料進去。所以中間要透過 Android 系統裡一個叫「**Health Connect**」的中繼站：PikoWalker 先把模擬走路產生的步數寫進 Health Connect；接著 Google 自己的「**Google Fit**」應用程式會去 Health Connect 把這些步數讀出來，加進 Google 帳號的每日步數；最後皮克敏 Bloom 再去讀 Google Fit 的步數，顯示在遊戲裡。三段是三個獨立的授權，缺一段，步數就傳不到下一段。
   ...
   **想馬上看到步數變化的話**：Google Fit 不是隨時都在背景同步，先自己打開一次 **Google Fit**（讓它主動去跟 Health Connect 要一次最新資料），再打開皮克敏 Bloom，遊戲通常就能馬上讀到新的步數，不用乾等。
...
**PikoWalker 顯示還在走，但遊戲裡的角色卻飄回真實位置不動了？**
先確認「電池最佳化」是「已停用」狀態。如果已經是已停用還是發生，把偽造GPS完全停止、再重新開始一次通常可以恢復；如果常常發生，麻煩照下面「回報問題」的方式抓紀錄給我看。

**打開 PikoWalker 卡在讀取畫面，或閃退回桌面？**
先完全關閉 PikoWalker（從最近使用列表滑掉）再重新打開一次，通常會自動恢復。如果重開還是卡住，把手機重新開機通常能解決；常常發生的話麻煩照下面「回報問題」的方式回報。

**Google Fit / Health Connect 步數沒有增加，或皮克敏 Bloom 裡步數沒變？**
照「開始使用前」第 4 項，確認三層都設定好了：PikoWalker 有寫入 Health Connect 的權限、Google Fit 有讀取 Health Connect 的權限、皮克敏 Bloom 的「步數」設定切成「使用 Google Fit 追蹤測量」。這是三個分開的授權/設定，缺一個步數都不會進遊戲。三層都設定好但還是沒看到變化的話，先打開一次 Google Fit 讓它跟 Health Connect 同步，再開皮克敏 Bloom。
